<div class="flex flex-col w-[300px] mx-auto py-16">
    <div class="flex gap-4 justify-between">
        <input type="text" wire:model="todoText" wire:keydown.enter="addTodo" placeholder="Type and hit enter"
            class="flex-1">
        <button class="py-2 px-4 bg-indigo-500 hover:bg-indigo-600 rounded text-white" wire:click="addTodo">Add
        </button>
    </div>
    ...
    <div class="py-6">
        ...
        @foreach ($todos as $index => $todo)
            <div class="flex gap-4 justify-between items-center py-1">
                ...
                <label class="flex-1 {{ $todo->is_done ? 'line-through' : '' }}">{{ $todo->todo }}</label>
